...
if [ "$RESTY_IMAGE_BASE" == "alpine" ]; then
  ...
  cp output/${KONG_PACKAGE_NAME}-${KONG_VERSION}.aws.rpm output/kong.rpm
	...
	cp output/${KONG_PACKAGE_NAME}-${KONG_VERSION}.rhel${RESTY_IMAGE_TAG}.${ARCHITECTURE}.rpm output/kong.rpm
...
--cache-from kong/kong-build-tools:test-$ARCHITECTURE-$RESTY_IMAGE_BASE-$RESTY_IMAGE_TAG \
-f test/$DOCKER_FILE \
